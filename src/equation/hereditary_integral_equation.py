...
class HereditaryIntegralEquation(ChengEquation):
    ...
    def viscoelastic_params(self):
        ...
        if self.model.viscoelastic_phase == "RELAXATION":
            ...
            f = lambda t: creep(np.zeros(t.shape))
            # f = lambda t: creep(t)
        else:
        ...
        M1 = 3 * K
        M2 = 1/f(time)
        beta1 = (3 * K * f(time) + 1) / (3 * K * f(time) + 2)
        beta2 = 1 / (3 * K * f(time) + 2)
        C = beta2
        return M1, M2, beta1, beta2, C
    ...
    def multiplier(self):
        ...
        return 1/(creep(self.time) - np.heaviside(self.time - 25, 1)*creep(self.time-25))
        # return np.ones(self.time.shape)

    def stiffness_domain(self, phi, point):
        M1, M2, LB1, LB2, B1, B2, space_size, time_size = self.cheng_params(phi, point)
        return (M1 * LB1 + LB2 * self.multiplier()).swapaxes(1, 2).reshape([2, 2 * space_size, time_size])
    ...
    def stiffness_boundary_neumann(self, phi, point):
        ...
        M2B2 = (B2 * self.multiplier()).swapaxes(0, 2).swapaxes(1, 3)
    ...
    def petrov_galerkin_stiffness_domain(self, phi, w, point):
        ...
        sigma = (M1 * B1 + B2 * self.multiplier()).swapaxes(0, 2).swapaxes(1, 3)
